...
export function ModeSelector({
  ...
  return (
    <section className="space-y-5" aria-labelledby="mode-selector-title">
        ...
        {SETUP_MODES.map((mode) => {
          ...
          return (
            <Card
              ...
              className={cn(
                "group min-h-44 cursor-pointer overflow-hidden border-border/70 bg-card/85 py-0 shadow-lg shadow-primary/5 transition-[border-color,background-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-xl focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/40 focus-visible:outline-none",
            ...
            >
              <CardContent className="flex h-full flex-col justify-between gap-6 p-5">
                <div className="flex items-start justify-between gap-3">
                  <div className="space-y-2">
                    <p className="text-xl font-black tracking-tight">{modeLabel}</p>
                    <p
                      className={cn(
